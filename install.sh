...
BASE_URL_DIST="${CU_BASE_URL:-https://github.com/CryoByte33/steam-deck-utilities}"
BASE_URL_CONTENT="${CU_BASE_URL_CONTENT:-https://raw.githubusercontent.com/CryoByte33/steam-deck-utilities}"
...
wget "$CU_BASE_URL/releases/download/latest/cu.sha256" -O "$HOME/.cryo_utilities/cu.sha256" 2>&1
sleep 1
if sha256 -c --quiet cu.sha256; then
...
for i in {1..3}; do
  # Download binary
  wget "$BASE_URL_DIST/releases/download/latest/cryo_utilities" -O "$HOME/.cryo_utilities/cryo_utilities" 2>&1 | sed -u 's/.* \([0-9]\+%\)\ \+\([0-9.]\+.\) \(.*\)/\1\n# Downloading at \2\/s, ETA \3/' | zenity --progress --title="Downloading CU Binary, attempt $i of 3..." --auto-close --width=500
...
rm -f cu.md5 cu.sha256 &>/dev/null

# Remove old launcher
rm -f "$HOME/.cryo_utilities/launcher.sh" &>/dev/null

# Install launcher script
wget $BASE_URL_CONTENT/main/launcher.sh -O "$HOME/.cryo_utilities/launcher.sh"
...
# Install Icon
wget $BASE_URL_CONTENT/main/cmd/cryoutilities/Icon.png -O "$HOME/.cryo_utilities/cryo-utilities.png"
...
Exec=curl $BASE_URL_CONTENT/main/uninstall.sh | bash -s --
Icon=delete
Terminal=false
Type=Application
...
Exec=curl $BASE_URL_CONTENT/main/install.sh | bash -s --
